...
return (object) array(
    ...
    'services' => (object) array(
        'mail' => (object) array(
            'Host' => $client->getValue("mailHost", ""),
            'SMTPAuth' => $client->getValue("mailSMTPAuth", true),
            'Username' => $client->getValue("mailUsername", ""),
            'Password' => $client->getValue("mailPassword", ""),
            'SMTPSecure' => $client->getValue("mailSMTPSecure", "ssl"),
            'Port' => $client->getValue("mailPort", 465),
            'From' => $client->getValue("mailFrom", ""),
        ),
        'recaptcha' => (object) array(
            'secret_key' => $client->getValue("recaptchaSecretKey", ""),
        ),
        'url' => (object) array(
            'signature' => $client->getValue("urlSignature", ""),
        ),
    ),
);
